...
var ASM_CONSTS = {
  4039634: () => { var inp = document.getElementById('mobile-keyboard-input'); if (inp) { inp.focus(); inp.click(); } },  
 4039737: () => { var inp = document.getElementById('mobile-keyboard-input'); if (inp) { inp.blur(); } document.getElementById('canvas').focus(); },  
 4039869: ($0) => { var str = UTF8ToString($0) + '\n\n' + 'Abort/Retry/Ignore/AlwaysIgnore? [ariA] :'; var reply = window.prompt(str, "i"); if (reply === null) { reply = "i"; } return reply.length === 1 ? reply.charCodeAt(0) : -1; },  
 4040084: () => { if (typeof(AudioContext) !== 'undefined') { return true; } else if (typeof(webkitAudioContext) !== 'undefined') { return true; } return false; },  
 4040231: () => { if ((typeof(navigator.mediaDevices) !== 'undefined') && (typeof(navigator.mediaDevices.getUserMedia) !== 'undefined')) { return true; } else if (typeof(navigator.webkitGetUserMedia) !== 'undefined') { return true; } return false; },  
 4040465: ($0) => { if(typeof(Module['SDL2']) === 'undefined') { Module['SDL2'] = {}; } var SDL2 = Module['SDL2']; if (!$0) { SDL2.audio = {}; } else { SDL2.capture = {}; } if (!SDL2.audioContext) { if (typeof(AudioContext) !== 'undefined') { SDL2.audioContext = new AudioContext(); } else if (typeof(webkitAudioContext) !== 'undefined') { SDL2.audioContext = new webkitAudioContext(); } if (SDL2.audioContext) { if ((typeof navigator.userActivation) === 'undefined') { autoResumeAudioContext(SDL2.audioContext); } } } return SDL2.audioContext === undefined ? -1 : 0; },  
 4041017: () => { var SDL2 = Module['SDL2']; return SDL2.audioContext.sampleRate; },  
 4041085: ($0, $1, $2, $3) => { var SDL2 = Module['SDL2']; var have_microphone = function(stream) { if (SDL2.capture.silenceTimer !== undefined) { clearInterval(SDL2.capture.silenceTimer); SDL2.capture.silenceTimer = undefined; SDL2.capture.silenceBuffer = undefined } SDL2.capture.mediaStreamNode = SDL2.audioContext.createMediaStreamSource(stream); SDL2.capture.scriptProcessorNode = SDL2.audioContext.createScriptProcessor($1, $0, 1); SDL2.capture.scriptProcessorNode.onaudioprocess = function(audioProcessingEvent) { if ((SDL2 === undefined) || (SDL2.capture === undefined)) { return; } audioProcessingEvent.outputBuffer.getChannelData(0).fill(0.0); SDL2.capture.currentCaptureBuffer = audioProcessingEvent.inputBuffer; dynCall('vp', $2, [$3]); }; SDL2.capture.mediaStreamNode.connect(SDL2.capture.scriptProcessorNode); SDL2.capture.scriptProcessorNode.connect(SDL2.audioContext.destination); SDL2.capture.stream = stream; }; var no_microphone = function(error) { }; SDL2.capture.silenceBuffer = SDL2.audioContext.createBuffer($0, $1, SDL2.audioContext.sampleRate); SDL2.capture.silenceBuffer.getChannelData(0).fill(0.0); var silence_callback = function() { SDL2.capture.currentCaptureBuffer = SDL2.capture.silenceBuffer; dynCall('vp', $2, [$3]); }; SDL2.capture.silenceTimer = setInterval(silence_callback, ($1 / SDL2.audioContext.sampleRate) * 1000); if ((navigator.mediaDevices !== undefined) && (navigator.mediaDevices.getUserMedia !== undefined)) { navigator.mediaDevices.getUserMedia({ audio: true, video: false }).then(have_microphone).catch(no_microphone); } else if (navigator.webkitGetUserMedia !== undefined) { navigator.webkitGetUserMedia({ audio: true, video: false }, have_microphone, no_microphone); } },  
 4042778: ($0, $1, $2, $3) => { var SDL2 = Module['SDL2']; SDL2.audio.scriptProcessorNode = SDL2.audioContext['createScriptProcessor']($1, 0, $0); SDL2.audio.scriptProcessorNode['onaudioprocess'] = function (e) { if ((SDL2 === undefined) || (SDL2.audio === undefined)) { return; } if (SDL2.audio.silenceTimer !== undefined) { clearInterval(SDL2.audio.silenceTimer); SDL2.audio.silenceTimer = undefined; SDL2.audio.silenceBuffer = undefined; } SDL2.audio.currentOutputBuffer = e['outputBuffer']; dynCall('vp', $2, [$3]); }; SDL2.audio.scriptProcessorNode['connect'](SDL2.audioContext['destination']); if (SDL2.audioContext.state === 'suspended') { SDL2.audio.silenceBuffer = SDL2.audioContext.createBuffer($0, $1, SDL2.audioContext.sampleRate); SDL2.audio.silenceBuffer.getChannelData(0).fill(0.0); var silence_callback = function() { if ((typeof navigator.userActivation) !== 'undefined') { if (navigator.userActivation.hasBeenActive) { SDL2.audioContext.resume(); } } SDL2.audio.currentOutputBuffer = SDL2.audio.silenceBuffer; dynCall('vp', $2, [$3]); SDL2.audio.currentOutputBuffer = undefined; }; SDL2.audio.silenceTimer = setInterval(silence_callback, ($1 / SDL2.audioContext.sampleRate) * 1000); } },  
 4043953: ($0, $1) => { var SDL2 = Module['SDL2']; var numChannels = SDL2.capture.currentCaptureBuffer.numberOfChannels; for (var c = 0; c < numChannels; ++c) { var channelData = SDL2.capture.currentCaptureBuffer.getChannelData(c); if (channelData.length != $1) { throw 'Web Audio capture buffer length mismatch! Destination size: ' + channelData.length + ' samples vs expected ' + $1 + ' samples!'; } if (numChannels == 1) { for (var j = 0; j < $1; ++j) { setValue($0 + (j * 4), channelData[j], 'float'); } } else { for (var j = 0; j < $1; ++j) { setValue($0 + (((j * numChannels) + c) * 4), channelData[j], 'float'); } } } },  
 4044558: ($0, $1) => { var SDL2 = Module['SDL2']; var buf = $0 >>> 2; var numChannels = SDL2.audio.currentOutputBuffer['numberOfChannels']; for (var c = 0; c < numChannels; ++c) { var channelData = SDL2.audio.currentOutputBuffer['getChannelData'](c); if (channelData.length != $1) { throw 'Web Audio output buffer length mismatch! Destination size: ' + channelData.length + ' samples vs expected ' + $1 + ' samples!'; } for (var j = 0; j < $1; ++j) { channelData[j] = HEAPF32[buf + (j*numChannels + c)]; } } },  
 4045047: ($0) => { var SDL2 = Module['SDL2']; if ($0) { if (SDL2.capture.silenceTimer !== undefined) { clearInterval(SDL2.capture.silenceTimer); } if (SDL2.capture.stream !== undefined) { var tracks = SDL2.capture.stream.getAudioTracks(); for (var i = 0; i < tracks.length; i++) { SDL2.capture.stream.removeTrack(tracks[i]); } } if (SDL2.capture.scriptProcessorNode !== undefined) { SDL2.capture.scriptProcessorNode.onaudioprocess = function(audioProcessingEvent) {}; SDL2.capture.scriptProcessorNode.disconnect(); } if (SDL2.capture.mediaStreamNode !== undefined) { SDL2.capture.mediaStreamNode.disconnect(); } SDL2.capture = undefined; } else { if (SDL2.audio.scriptProcessorNode != undefined) { SDL2.audio.scriptProcessorNode.disconnect(); } if (SDL2.audio.silenceTimer !== undefined) { clearInterval(SDL2.audio.silenceTimer); } SDL2.audio = undefined; } if ((SDL2.audioContext !== undefined) && (SDL2.audio === undefined) && (SDL2.capture === undefined)) { SDL2.audioContext.close(); SDL2.audioContext = undefined; } },  
 4046053: ($0) => { window.open(UTF8ToString($0), "_blank") },  
 4046093: ($0, $1, $2) => { var w = $0; var h = $1; var pixels = $2; if (!Module['SDL2']) Module['SDL2'] = {}; var SDL2 = Module['SDL2']; if (SDL2.ctxCanvas !== Module['canvas']) { SDL2.ctx = Browser.createContext(Module['canvas'], false, true); SDL2.ctxCanvas = Module['canvas']; } if (SDL2.w !== w || SDL2.h !== h || SDL2.imageCtx !== SDL2.ctx) { SDL2.image = SDL2.ctx.createImageData(w, h); SDL2.w = w; SDL2.h = h; SDL2.imageCtx = SDL2.ctx; } var data = SDL2.image.data; var src = pixels / 4; var dst = 0; var num; if (typeof CanvasPixelArray !== 'undefined' && data instanceof CanvasPixelArray) { num = data.length; while (dst < num) { var val = HEAP32[src]; data[dst ] = val & 0xff; data[dst+1] = (val >> 8) & 0xff; data[dst+2] = (val >> 16) & 0xff; data[dst+3] = 0xff; src++; dst += 4; } } else { if (SDL2.data32Data !== data) { SDL2.data32 = new Int32Array(data.buffer); SDL2.data8 = new Uint8Array(data.buffer); SDL2.data32Data = data; } var data32 = SDL2.data32; num = data32.length; data32.set(HEAP32.subarray(src, src + num)); var data8 = SDL2.data8; var i = 3; var j = i + 4*num; if (num % 8 == 0) { while (i < j) { data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; } } else { while (i < j) { data8[i] = 0xff; i = i + 4 | 0; } } } SDL2.ctx.putImageData(SDL2.image, 0, 0); },  
 4047559: ($0, $1, $2, $3, $4) => { var w = $0; var h = $1; var hot_x = $2; var hot_y = $3; var pixels = $4; var canvas = document.createElement("canvas"); canvas.width = w; canvas.height = h; var ctx = canvas.getContext("2d"); var image = ctx.createImageData(w, h); var data = image.data; var src = pixels / 4; var dst = 0; var num; if (typeof CanvasPixelArray !== 'undefined' && data instanceof CanvasPixelArray) { num = data.length; while (dst < num) { var val = HEAP32[src]; data[dst ] = val & 0xff; data[dst+1] = (val >> 8) & 0xff; data[dst+2] = (val >> 16) & 0xff; data[dst+3] = (val >> 24) & 0xff; src++; dst += 4; } } else { var data32 = new Int32Array(data.buffer); num = data32.length; data32.set(HEAP32.subarray(src, src + num)); } ctx.putImageData(image, 0, 0); var url = hot_x === 0 && hot_y === 0 ? "url(" + canvas.toDataURL() + "), auto" : "url(" + canvas.toDataURL() + ") " + hot_x + " " + hot_y + ", auto"; var urlBuf = _malloc(url.length + 1); stringToUTF8(url, urlBuf, url.length + 1); return urlBuf; },  
 4048547: ($0) => { if (Module['canvas']) { Module['canvas'].style['cursor'] = UTF8ToString($0); } },  
 4048630: () => { if (Module['canvas']) { Module['canvas'].style['cursor'] = 'none'; } },  
 4048699: () => { return window.innerWidth; },  
 4048729: () => { return window.innerHeight; },  
 4048760: ($0, $1) => { alert(UTF8ToString($0) + "\n\n" + UTF8ToString($1)); }
};
function web_touch_active() { return (typeof TouchGamepad !== 'undefined' && TouchGamepad.isActive()) ? 1 : 0; }
function web_touch_stick_x() { return (typeof TouchGamepad !== 'undefined') ? TouchGamepad.getStickX() : 0; }
function web_touch_stick_y() { return (typeof TouchGamepad !== 'undefined') ? TouchGamepad.getStickY() : 0; }
function web_touch_buttons() { if (typeof TouchGamepad === 'undefined' || !TouchGamepad.isActive()) return 0; var b = 0; if (TouchGamepad.isButtonPressed('tb-a')) b |= 0x8000; if (TouchGamepad.isButtonPressed('tb-b')) b |= 0x4000; if (TouchGamepad.isButtonPressed('tb-z')) b |= 0x2000; if (TouchGamepad.isButtonPressed('tb-start')) b |= 0x1000; if (TouchGamepad.isButtonPressed('tb-l')) b |= 0x0020; if (TouchGamepad.isButtonPressed('tb-r')) b |= 0x0010; if (TouchGamepad.isButtonPressed('tb-cu')) b |= 0x0008; if (TouchGamepad.isButtonPressed('tb-cd')) b |= 0x0004; if (TouchGamepad.isButtonPressed('tb-cl')) b |= 0x0002; if (TouchGamepad.isButtonPressed('tb-cr')) b |= 0x0001; return b; }
function web_has_anchor_config() { return (typeof window._anchorConfig !== 'undefined') ? 1 : 0; }
function web_anchor_config_get(key) { if (typeof window._anchorConfig === 'undefined') return 0; var k = UTF8ToString(key); var val = window._anchorConfig[k] || ''; var len = lengthBytesUTF8(val) + 1; var ptr = _malloc(len); stringToUTF8(val, ptr, len); return ptr; }
function web_dev_cvars_get() { var s = (typeof window._devCvars === 'string') ? window._devCvars : ''; var len = lengthBytesUTF8(s) + 1; var ptr = _malloc(len); stringToUTF8(s, ptr, len); return ptr; }
function web_mount_idbfs() { var dirs = ["/Save"]; for (var i = 0; i < dirs.length; i++) { try { FS.mkdir(dirs[i]); } catch (e) { } FS.mount(IDBFS, {}, dirs[i]); } FS.syncfs(true, function(err) { if (err) { console.error("[Web] IDBFS load failed:", err); } else { console.log("[Web] IDBFS loaded. Files in /Save/:"); try { var files = FS.readdir("/Save"); console.log("[Web]   " + files.join(", ")); } catch(e) {} } setValue(_web_idbfs_ready_ptr(), 1, 'i32'); }); }
...
var _web_save_to_idb = Module['_web_save_to_idb'] = makeInvalidEarlyAccess('_web_save_to_idb');
var _web_select_warp = Module['_web_select_warp'] = makeInvalidEarlyAccess('_web_select_warp');
var _web_console = Module['_web_console'] = makeInvalidEarlyAccess('_web_console');
var _web_scene = Module['_web_scene'] = makeInvalidEarlyAccess('_web_scene');
var _web_player_pos = Module['_web_player_pos'] = makeInvalidEarlyAccess('_web_player_pos');
var _web_wants_text_input = Module['_web_wants_text_input'] = makeInvalidEarlyAccess('_web_wants_text_input');
var _web_otr_loaded = Module['_web_otr_loaded'] = makeInvalidEarlyAccess('_web_otr_loaded');
var _web_get_otr_status = Module['_web_get_otr_status'] = makeInvalidEarlyAccess('_web_get_otr_status');
var _web_idbfs_ready_ptr = Module['_web_idbfs_ready_ptr'] = makeInvalidEarlyAccess('_web_idbfs_ready_ptr');
var _web_fs_init = Module['_web_fs_init'] = makeInvalidEarlyAccess('_web_fs_init');
var _web_is_idbfs_ready = Module['_web_is_idbfs_ready'] = makeInvalidEarlyAccess('_web_is_idbfs_ready');
var _web_extract_rom = Module['_web_extract_rom'] = makeInvalidEarlyAccess('_web_extract_rom');
var _web_get_rom_version = Module['_web_get_rom_version'] = makeInvalidEarlyAccess('_web_get_rom_version');
var _web_configure_anchor = Module['_web_configure_anchor'] = makeInvalidEarlyAccess('_web_configure_anchor');
...
function assignWasmExports(wasmExports) {
  ...
  assert(typeof wasmExports['web_save_to_idb'] != 'undefined', 'missing Wasm export: web_save_to_idb');
  assert(typeof wasmExports['web_select_warp'] != 'undefined', 'missing Wasm export: web_select_warp');
  assert(typeof wasmExports['web_console'] != 'undefined', 'missing Wasm export: web_console');
  assert(typeof wasmExports['web_scene'] != 'undefined', 'missing Wasm export: web_scene');
  assert(typeof wasmExports['web_player_pos'] != 'undefined', 'missing Wasm export: web_player_pos');
  assert(typeof wasmExports['web_wants_text_input'] != 'undefined', 'missing Wasm export: web_wants_text_input');
  assert(typeof wasmExports['web_otr_loaded'] != 'undefined', 'missing Wasm export: web_otr_loaded');
  assert(typeof wasmExports['web_get_otr_status'] != 'undefined', 'missing Wasm export: web_get_otr_status');
  assert(typeof wasmExports['web_idbfs_ready_ptr'] != 'undefined', 'missing Wasm export: web_idbfs_ready_ptr');
  assert(typeof wasmExports['web_fs_init'] != 'undefined', 'missing Wasm export: web_fs_init');
  assert(typeof wasmExports['web_is_idbfs_ready'] != 'undefined', 'missing Wasm export: web_is_idbfs_ready');
  assert(typeof wasmExports['web_extract_rom'] != 'undefined', 'missing Wasm export: web_extract_rom');
  assert(typeof wasmExports['web_get_rom_version'] != 'undefined', 'missing Wasm export: web_get_rom_version');
  assert(typeof wasmExports['web_configure_anchor'] != 'undefined', 'missing Wasm export: web_configure_anchor');
  ...
  _web_save_to_idb = Module['_web_save_to_idb'] = createExportWrapper('web_save_to_idb', wasmExports['web_save_to_idb'], 0);
  _web_select_warp = Module['_web_select_warp'] = createExportWrapper('web_select_warp', wasmExports['web_select_warp'], 1);
  _web_console = Module['_web_console'] = createExportWrapper('web_console', wasmExports['web_console'], 1);
  _web_scene = Module['_web_scene'] = createExportWrapper('web_scene', wasmExports['web_scene'], 0);
  _web_player_pos = Module['_web_player_pos'] = createExportWrapper('web_player_pos', wasmExports['web_player_pos'], 0);
  ...
  _web_otr_loaded = Module['_web_otr_loaded'] = createExportWrapper('web_otr_loaded', wasmExports['web_otr_loaded'], 0);
  _web_get_otr_status = Module['_web_get_otr_status'] = createExportWrapper('web_get_otr_status', wasmExports['web_get_otr_status'], 0);
  _web_idbfs_ready_ptr = Module['_web_idbfs_ready_ptr'] = createExportWrapper('web_idbfs_ready_ptr', wasmExports['web_idbfs_ready_ptr'], 0);
  _web_fs_init = Module['_web_fs_init'] = createExportWrapper('web_fs_init', wasmExports['web_fs_init'], 0);
  _web_is_idbfs_ready = Module['_web_is_idbfs_ready'] = createExportWrapper('web_is_idbfs_ready', wasmExports['web_is_idbfs_ready'], 0);
  _web_extract_rom = Module['_web_extract_rom'] = createExportWrapper('web_extract_rom', wasmExports['web_extract_rom'], 2);
  _web_get_rom_version = Module['_web_get_rom_version'] = createExportWrapper('web_get_rom_version', wasmExports['web_get_rom_version'], 1);
  _web_configure_anchor = Module['_web_configure_anchor'] = createExportWrapper('web_configure_anchor', wasmExports['web_configure_anchor'], 4);
...
var wasmImports = {
  ...
  /** @export */
  web_anchor_config_get,
  /** @export */
  web_dev_cvars_get,
  /** @export */
  web_has_anchor_config,
  /** @export */
  web_mount_idbfs,
  /** @export */
  web_touch_active,
